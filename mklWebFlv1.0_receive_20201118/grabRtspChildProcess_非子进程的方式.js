...
module.exports = function (msgObj) {
    ...
    var _baseDirName = global._baseDirname;
    ...
    var outPutPath = path.join(global._hlsCacheDir, token);
    ...
    var _currTime = new Date().format('yMdhms');
    outPutPath = path.join(outPutPath, _currTime);
    if (!fs.existsSync(outPutPath)) {
        fs.mkdirSync(outPutPath);
    }
    ...
    var ffmpegCmdStr = ffmpegCmdName + ' -i ' + flvUrl + ' -force_key_frames "expr:gte(t,n_forced*10)" -c copy -map 0 -f ssegment -segment_format mpegts -segment_list ' +
        m3u8Path + ' -segment_list_flags +live -segment_list_size 6 -segment_time 10 ' + tsPath;
    ...
    var spawnStr=' -i ' + flvUrl + ' -force_key_frames "expr:gte(t,n_forced*10)" -c copy -map 0 -f ssegment -segment_format mpegts -segment_list ' +
        m3u8Path + ' -segment_list_flags +live -segment_list_size 6 -segment_time 10 ' + tsPath;
    var cmdProcess = childProcess.spawn('ffmpeg',[spawnStr]);
